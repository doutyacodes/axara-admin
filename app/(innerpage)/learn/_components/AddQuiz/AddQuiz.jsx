...
const AddQuiz = () => {
  ...
  const validateForm = () => {
    if (!formData.subjectId || !formData.grade || !formData.startDate || !formData.endDate) {
...
// 
        if (err.response?.status === 400 && err.response?.data?.message) {
          ...
          if (errorMsg.includes("already")) {
            ...
            // });
            toast.error(`A test is already scheduled Date range of ${err.response.data.dateRange}. Please choose another date.`)
          } else {
